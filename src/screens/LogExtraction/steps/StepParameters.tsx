    ...
    <div className="card" style={{ padding: 0 }}>
      <button type="button" onClick={() => setOpen(!open)} className="row row-between" style={{ background: 'transparent', border: 'none', width: '100%', padding: '20px 24px', cursor: 'pointer', textAlign: 'left' }}>
        <div className="row row-gap-12">
          ...
          <span style={{ fontFamily: 'var(--serif)', fontSize: 16 }}>{t('extraction.steps.parameters.algorithmAssist')}</span>
        </div>
            ...
            {ey !== null && zone && (
              <div>
                <div className="row row-between" style={{ marginBottom: 8 }}>
                  ...
                  <span className="t-mono" style={{ color: zone.color, fontSize: 13, fontWeight: 600 }}>{ey.toFixed(1)}% · {t(`extraction.zones.${zone.labelKey}`)}</span>
                </div>
    ...
    <div>
      <div className="step-meta" style={{ marginBottom: 20 }}>
        <div className="col col-gap-4">
          <h2 className="h-display" style={{ fontSize: 28, margin: 0 }}>{t('extraction.steps.parameters.title')}</h2>
          <span className="t-sec" style={{ fontSize: 12 }}>{t(`methods.${methodById(method).id}`, { defaultValue: methodById(method).name }).toUpperCase()}</span>
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: 24 }}>
        <div className="col col-gap-24">
          <div className="grid grid-2">
            <Field label={t('extraction.fields.dose')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>g</span>}>
              <Input large type="number" value={draft.dose} onChange={e => update({ dose: parseFloat(e.target.value) || 0 })} />
            </Field>
            <Field label={t('extraction.fields.yield')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>g</span>}>
          ...
          <div className="grid grid-2">
            <Field label={t('extraction.fields.extractionTime')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>m:ss</span>}>
              <Input value={fmtTime(draft.timeS)} onChange={e => handleTimeChange(e.target.value)} />
            </Field>
            <Field label={t('extraction.fields.temperature')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>°C</span>}>
              <Input type="number" value={draft.temp} onChange={e => update({ temp: parseFloat(e.target.value) || 0 })} />
            </Field>
          </div>
          {isEspresso && (
            <Field label={t('extraction.fields.pressure')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>bar</span>}>
          ...
          <div>
            <label className="row row-gap-8" style={{ cursor: 'pointer', userSelect: 'none' }}>
              ...
              <span className="t-sec" style={{ fontSize: 12 }}>{t('extraction.steps.parameters.refractometer')}</span>
            </label>
            {draft.showTds && (
              <div style={{ marginTop: 12 }}>
                <Field label={t('extraction.fields.tds')} right={<span className="t-ter t-mono" style={{ fontSize: 11 }}>%</span>}>
