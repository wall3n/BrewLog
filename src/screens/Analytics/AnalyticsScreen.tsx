...
function LineChart({ data }: { data: { rating: number }[] }) {
  ...
  if (data.length === 0) return <div className="t-sec" style={{ fontSize: 12, padding: 12 }}>{t('analytics.logFirst')}</div>;
    ...
    <div>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div className="row row-between" style={{ marginBottom: 16 }}>
          ...
          <span className="t-mono t-sec" style={{ fontSize: 11 }}>{t('analytics.last', { count: ratingsOverTime.length })}</span>
        </div>
        <LineChart data={ratingsOverTime} />
      </div>

      <div className="card" style={{ marginBottom: 16 }}>
        <div className="t-upper" style={{ marginBottom: 16 }}>{t('analytics.methodDistribution')}</div>
        <div className="col col-gap-12">
          {methodList.map(([m, c]) => (
            <div key={m}>
              <div className="row row-between" style={{ marginBottom: 6 }}>
                <span style={{ fontSize: 12 }}>{t(`methods.${m}`, { defaultValue: m })}</span>
      ...
      <div className="card">
        <div className="t-upper" style={{ marginBottom: 16 }}>{t('analytics.topBeans')}</div>
        <div className="col col-gap-12">
          {topBeans.length === 0
